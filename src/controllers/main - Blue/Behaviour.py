...
class Behaviour(Detection, Drive, Gps, Grabber, Communication):
    ...
    def __init__(self):
        ...
        #indicates the current block to look for
        self.blockToFind = 0
    ...
    def setBlockToFind(self):
        """finds the nearest block in blockLocations, only considering blue or unknown blocks
        returns the distance away if there are ble or uknown blocks and 0 if there are no blocks/all are red"""
        magnitudes = []
        for b in self.blockLocations:
            if(b[2] == 0 or b[2] == 2):
                b = [b[0] - self.mid_position()[0], b[1] - self.mid_position()[1]]
    ...
    def findBlocks2(self):
        ...
        if self.state == [1,2]:
            if(self.checkForBlock() == "Done"):
                ...
                self.state = [0,2]
        #[2,1] if block is red, first reverse
        if self.state == [2,1]:
            self.backwards(5)
            self.count += 1
            if(self.count > 10):
